...
void blur_bitmap(int passes, const uint8_t *src, uint8_t *dst, int stride, int w, int h, int radius, int horz_only)
{
   ...
   uint32_t *col_buf = horz_only ? NULL : (uint32_t *)malloc((size_t)h * 4 * sizeof(uint32_t));
   if (!horz_only && !col_buf)
   {
      free(tmp);
      return;
   }
   ...
   for (int pass = 0; pass < passes; pass++)
   {
      ...
      // --- Vertical pass: wr → wr in-place (skipped if horz_only) ---
      if (!horz_only)
      {
